...
# Adjust the import path if your CLI script is not directly in the root or accessible via this path
# This assumes self_promise_cli.py is in the parent directory of the tests directory
...
def mock_webbrowser_open(monkeypatch):
    ...
    def mock_open(url, new=0, autoraise=True):
        ...
        click.echo(f"Mocked webbrowser.open called with URL: {url}")
        return True
...
def test_create_promise_auto_evidence_no_tracker_connect_yes(runner, mock_config_dir, mock_deployed_addresses, mock_webbrowser_open):
...
def test_submit_evidence(runner, tmp_path, mock_deployed_addresses):
    ...
    result = runner.invoke(cli, ['submit-evidence', '--promise-id', 'mock_promise_456', '--evidence-file', str(evidence_file)])
    assert result.exit_code == 0
